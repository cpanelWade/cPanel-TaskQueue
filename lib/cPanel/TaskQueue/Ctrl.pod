# cpanel - cPanel/TaskQueue/Ctrl.pod              Copyright(c) 2011 cPanel, Inc.
#                                                           All rights Reserved.
...
This document describes cPanel::TaskQueue::Ctrl version 0.504
...
=head2 unqueue_tasks( $fh, $queue, $sched, @task_ids )
...
=head2 list_tasks( $fh, $queue, $sched, @options )
...
=head2 find_task( $fh, $queue, $sched, $subcmd, $match )
...
=head2 list_plugins( $fh, $queue, $sched, $option )
...
=head2 list_commands( $fh, $queue, $sched )

Print the names of the commands supported by the loaded plugins.

=head2 schedule_tasks( $fh, $queue, $sched, [ $subcmd, $value, ] @cmds )
...
=head2 unschedule_tasks( $fh, $queue, $sched, @ids )
...
=head2 queue_status( $fh, $queue, $sched )

Display a summary of information about the C<$queue> and C<$sched>.

=head2 convert_state_files( $fh, $queue, $sched, $fmt )

Convert the state files for the C<$queue> and C<$sched> to the format described
by C<$fmt> and exit the program. At the end of conversion, C<die> with an indication
that we should restart the program. The C<$queue> and C<$sched> objects are not safe
