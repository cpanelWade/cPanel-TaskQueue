...
eval {
    cPanel::TaskQueue::PluginManager::load_plugins( '/xyzzy' );
};
...
eval {
    cPanel::TaskQueue::PluginManager::load_plugins( $plugindir );
};
...
eval {
    cPanel::TaskQueue::PluginManager::load_plugins( $plugindir, 'This is not a namespace' );
};
...
like( $logger_output, qr/Failed to load 'cPanel::FakeTasks::BadLoad'/,
    'BadLoad failure correctly detected.' );
like( $logger_output, qr/'cPanel::FakeTasks::NoRegister' not registered, no 'to_register/,
    'NoRegister failure correctly detected.' );
like( $logger_output, qr/'cPanel::FakeTasks::BadRegister2': invalid registration/,
    'BadRegister2 failure correctly detected.' );
like( $logger_output, qr/'cPanel::FakeTasks::BadRegister': invalid registration/,
